...
function renderSessionHeader(options: {
  ...
  pathWrapper.className = "chat-path-wrapper";
  ...
  copyBtn.className = "copy-path-button";
  copyBtn.type = "button";
  ...
  copyBtn.addEventListener("click", () => {
    navigator.clipboard.writeText(descriptor.primaryPath).then(() => {
      copyBtn.setAttribute("data-state", "success");
      const originalHtml = copyBtn.innerHTML;
      copyBtn.innerHTML = `
        <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor">
          <path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.751.751 0 0 1 .018-1.042.751.751 0 0 1 1.082-.018L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0Z"></path>
        </svg>
      `;
      setTimeout(() => {
      ...
      copyBtn.setAttribute("data-state", "error");
      setTimeout(() => copyBtn.removeAttribute("data-state"), 2000);
    });
  });

  pathWrapper.append(path, copyBtn);
